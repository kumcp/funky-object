    ...
    const promiseTypeMap = {
        0: (...params) => {
            ...
            return new Promise((resolve, reject) => {
                ...
                let result = func.apply(options.context, [...params.slice(0, position), callback, ...params.slice(position, params.length)])
                if (result) {
                    resolve(result)
                }
            })
        ...
        1: (...params) => {
            ...
            return new Promise((resolve, reject) => {
                ...
                func.apply(options.context, [...params.slice(0, position), callback, ...params.slice(position, params.length)])
            })
        },
        2: (...params) => {
            const lastArgs = params[params.length - 1]
            const nearLastArgs = params[params.length - 2]
            ...
            return new Promise((resolve, reject) => {
                ...
                func.apply(options.context, [...params.slice(0, position), successCallback, errorCallback, ...params.slice(position, params.length)])
            })
        }
    }
